(function(global){
  ...
  function renderPanel(state){
    return `
      <div class="MonthPicker__head">
        <button type="button" aria-label="上一年" title="上一年" onclick="MonthPicker.moveYear('${escapeHtml(state.id)}',-1,event)">‹</button>
        <strong>${state.viewYear}年</strong>
        <button type="button" aria-label="下一年" title="下一年" onclick="MonthPicker.moveYear('${escapeHtml(state.id)}',1,event)">›</button>
      </div>
      <div class="MonthPicker__grid">
        ${Array.from({length:12},(_,index)=>index+1).map(month=>{
          ...
          return `<button type="button" class="${selected?"selected":""} ${disabled?"disabled":""}" ${disabled?"disabled":""} onclick="MonthPicker.select('${escapeHtml(state.id)}','${monthValue}',event)"><span>${month}月</span></button>`;
        }).join("")}
      </div>`;
  }
  ...
  function sync(id){
    ...
    if(value)value.textContent=state.value?formatLabel(state.value):state.placeholder;
  }
