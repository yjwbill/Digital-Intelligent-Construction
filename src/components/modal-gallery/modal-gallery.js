(function(global){
  ...
  function renderStage(type,item){
    ...
    return `<section class="modal-gallery-stage ${type}"><div class="modal-gallery-window"><header><strong>${item.title}</strong><i>×</i></header><main>${type==="immersive"?"报告 / 图片预览画布":"弹框正文内容区域"}</main><footer><button>取消</button><button>确定</button></footer></div></section>`;
  }
  function open(type){
    close();
