...
function formatEconomyReportCnyAmount(value){return `CNY ${(Number(value)/10).toLocaleString("en",{minimumFractionDigits:2,maximumFractionDigits:3})} billion`;}
...
function formatEconomyReportSunburstValue(value,unit){
  ...
  return isEconomyReportEnglish()?formatEconomyReportCnyAmount(value):`${Number(value).toFixed(2)}亿元`;
}
...
function renderEconomyReportSlide(index){
  ...
  if(index===1){
    ...
    const appendix=isEconomyReportEnglish()?`<p>Appendix 1: STEC Project Economic Risk Inspection Report (${economyDashboardState.month.replace('-','')})</p><p>Appendix 2: Summary of Onboarded Projects among the Top Ten Annual Budget Profit Contributors</p>`:`<p>附件一：隧道股份工程项目经济风险月度检验单（${economyDashboardState.month.replace('-','')}）</p><p>附件二：纳入“年度全面预算利润贡献前十位范围”的已上线项目情况汇总表</p>`;
  ...
  if(index===6){const reportYear=Number(String(economyDashboardState.month||"2026-06").slice(0,4))||2026,budgetYear=String(reportYear-1).slice(-2),summary=economyReportText(`<p>结合隧道股份已上线单位 <b class="danger">${budgetYear}年</b> 全面预算营收安排，纳入对应单位利润贡献前十位范围的已上线项目个数共 <b class="danger">10</b> 个：</p><ol><li>上海隧道纳入利润贡献前十位范围的项目个数 <b class="danger">3</b> 个；</li><li>市政集团纳入利润贡献前十位范围的项目个数 <b class="danger">5</b> 个；</li><li>上海路桥纳入利润贡献前十位范围的项目个数 <b class="danger">2</b> 个。</li></ol>`,`<p>Based on STEC's <b class="danger">20${budgetYear}</b> annual revenue budget, <b class="danger">10</b> onboarded projects rank among their respective entities' top ten profit contributors:</p><ol><li><b class="danger">3</b> from Shanghai Tunnel;</li><li><b class="danger">5</b> from Municipal Engineering Group;</li><li><b class="danger">2</b> from Shanghai Road & Bridge.</li></ol>`);return `<section class="eco-report-slide report-page secondary-warning-report-page series-five-report-page">${renderEconomyReportPageHead("系列五","纳入年度全面预算利润贡献前十位范围的已上线项目经济指标汇总",6)}<div class="eco-report-copy series-five-summary">${summary}</div><div class="eco-report-table"><table><thead><tr><th>序号</th><th>单位</th><th>目标值（万元）</th><th>实际值（万元）</th><th>完成进度</th><th>目标利润率</th><th>存货率</th><th>偏离度</th></tr></thead><tbody>${[1,2,3,4].map((x,i)=>`<tr><td>${x}</td><td>${economyReportCompanies[i+1]}</td><td>2,281,877.92</td><td>667,300.24</td><td>${37+i*8}.66%</td><td>12.93%</td><td>${i===0?'-11.43%':'5.61%'}</td><td>${i===0?'-24.36%':'-7.53%'}</td></tr>`).join("")}<tr class="total"><td></td><td>合计</td><td>33,082,575.82</td><td>5,412,691.93</td><td></td><td>7.75%</td><td>-0.22%</td><td>-7.53%</td></tr></tbody></table></div><div class="eco-report-two-charts finance"><article><h3>当年营收完成进度（亿元）</h3>${renderEconomyReportRevenueProgress()}</article><article><h3>加权平均目标利润率与加权平均存货率</h3>${renderEconomyReportWeightedRates()}</article></div></section>`;}
  {const companyLabels=["上海隧道","市政集团","上海路桥","城建集团","城市环境","运营集团","城建设计","数字集团"],summary=economyReportText(`<p>根据隧道股份近两期预警情况展示，隧道股份 <b class="danger">11/17</b> 预警项目数量较上期 <b class="danger">整体持平</b>，主要变化：</p><p>一级指标方面：未整改黑色预警 <b class="danger">50</b> 个，前三名分别为 <b class="danger">合同预警、潜亏预警、结算预警</b>；红色预警 <b class="danger">新增5个</b>，橙色预警 <b class="danger">减少5个</b>，黄色预警 <b class="danger">新增1个</b>，蓝色预警 <b class="danger">整体持平</b>。</p><p>二级指标方面：本期二级预警 <b class="danger">新增5个</b>，前三名分别为 <b class="danger">资金存货关联、拖欠款预警、项目管理费</b>。</p>`,`<p>STEC's warning results for the latest two periods show that the number of warning projects on <b class="danger">11/17</b> was <b class="danger">unchanged overall</b> from the previous period. Key changes were:</p><p>Primary indicators: <b class="danger">50</b> overdue warnings. The top three were <b class="danger">Contract Warning, Potential Loss Warning, and Settlement Warning</b>. Red warnings increased by <b class="danger">5</b>, orange warnings decreased by <b class="danger">5</b>, yellow warnings increased by <b class="danger">1</b>, and blue warnings were unchanged.</p><p>Secondary indicators: <b class="danger">5</b> new warnings. The top three were <b class="danger">Fund-Inventory Correlation, Arrears Warning, and Project Management Fee</b>.</p>`);return `<section class="eco-report-slide report-page series-four-report-page">${renderEconomyReportPageHead("系列四","近五期预警情况对比",7)}<div class="eco-report-copy series-four-summary">${summary}</div><div class="eco-report-two-charts trend company-warning-chart"><article><h3>${economyReportTranslated("一级预警指标对比")}</h3>${renderEconomyReportStackedChart(2,companyLabels)}${renderEconomyReportLegend()}</article><article><h3>${economyReportTranslated("二级预警指标对比")}</h3>${renderEconomyReportSecondaryTrend()}</article></div></section>`;}
}
