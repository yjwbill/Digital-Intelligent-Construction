...
function renderEconomyDiagnosisTaskSmsCount(row,type){const batch=getEconomyDiagnosisTaskBatch(row),value=type==="sent"?row.sent:row.generated;return batch?.batchNo?`<a class="link" onclick="openSendRecordDrilldown('${'${'}escapeAttr(batch.batchNo)}','sent')">${'${'}value}</a>`:`<span class="link">${'${'}value}</span>`;}
