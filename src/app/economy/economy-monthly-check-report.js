...
function getEconomyMonthlyCheckCell(row,column,index){
  ...
  const seed=rowSeed+economyMonthlyCheckColumns.indexOf(column)*3;
...
function getEconomyMonthlyCheckRows(){
  ...
  return rows.length?rows:(typeof getEconomyEditionProjects==="function"?getEconomyEditionProjects():[]);
}
...
function getEconomyMonthlyCheckStats(projects){
  const warningProjects=projects.filter((row,index)=>economyMonthlyCheckColumns.some(column=>getEconomyMonthlyCheckCell(row,column,index).color));
  const unresolved=warningProjects.filter((row,index)=>economyMonthlyCheckColumns.some(column=>getEconomyMonthlyCheckCell(row,column,index).overdue));
...
function renderEconomyMonthlyCheckCompany(group,groupIndex){
  ...
  return `<section class="economy-monthly-company" id="economyMonthlyCompany${groupIndex}">
    <div class="economy-monthly-company-head">
    ...
    <div class="economy-monthly-table-wrap"><table>${renderEconomyMonthlyCheckHeader()}<tbody>${group.projects.map((row,index)=>{
      const seed=getEconomyMonthlyCheckSeed(row,index);
      const profit=(.5+(seed%145)/10).toFixed(1);
      return `<tr><td class="sticky-seq">${index+1}</td><td class="sticky-company">${group.company}</td><td class="sticky-branch">${row.branch||"-"}</td><td class="sticky-project"><button type="button" title="查看项目经济总览" onclick="openEconomyDiagnosisProjectOverview('${escapeAttr(row.sourceProjectId)}')">${row.projectName}</button></td><td class="number" title="${Number(row.contractAmount||0).toLocaleString('zh-CN',{minimumFractionDigits:2,maximumFractionDigits:2})} 万元">${Math.round(Number(row.contractAmount||0))}</td><td>${profit}%</td><td>${Math.round(Number(row.outputProgress||0))}%</td>${economyMonthlyCheckColumns.map(column=>{const cell=getEconomyMonthlyCheckCell(row,column,index);return `<td class="warning-cell ${cell.unavailable?'unavailable':''}">${renderEconomyMonthlyThunder(cell,column)}</td>`;}).join("")}</tr>`;
...
function renderEconomyMonthlyCheckReport(){
  ...
  return `<div class="economy-monthly-check-report">
    <header class="economy-monthly-check-toolbar">
      <div><h1>企业工程项目经济风险月度检验单</h1><p>${(economyDashboardState.month||"2026-06").replace("-","年")}月</p></div>
...
function openEconomyMonthlyCheckReport(){
  openModal("月度检验单",renderEconomyMonthlyCheckReport(),`<button class="btn" onclick="closeModal()">关闭</button><button class="btn primary" onclick="showToast('月度检验单导出成功')"><span aria-hidden="true">⇩</span> 导出</button>`,"large");
