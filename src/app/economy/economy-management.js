...
2）影响系统取值的字段：EAS编号（本项目）、子公司项目编号
3）影响大屏及诊断结果汇总统计的字段：所在省市、区域市场、项目类型、项目实施模式
请仔细检查以上字段是否完善。为保证诊断结果真实可信，财务、合同、主材、工期等相关信息也请及时更新。`;
...
function renderEconomyManagementManagedTable(rows){
  ...
  const columnKey=economyManagementState.tab==="ended"?"economyManagementEnded":"economyManagementManaged";
...
function renderEconomyManagementEndedTable(rows){
  ...
  return html.replace("economy-management-managed-table-card","economy-management-managed-table-card economy-management-ended-table-card").replace(/cancelEconomyManagement/g,"restoreEconomyManagement").replace(/结束纳管/g,"恢复纳管").replace("恢复纳管项目列表","结束纳管项目列表").replace(/<button class="link" onclick="openEconomyManagementModal\('[^']+'\)">编辑<\/button>/g,"");
}
...
function renderEconomyManagementEndedTable(rows){
  const html=renderEconomyManagementManagedTable(rows);
  return html.replace(/cancelEconomyManagement/g,"restoreEconomyManagement").replace(/结束纳管/g,"恢复纳管").replace("恢复纳管项目列表","结束纳管项目列表").replace(/<button class="link" onclick="openEconomyManagementModal\('[^']+'\)">编辑<\/button>/g,"");
}
