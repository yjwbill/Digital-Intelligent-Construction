...
tableColumnDefinitions.economyInitiationProjects=[
  ...
  {key:"manager",title:"项目经理",width:120,align:"center",render:x=>x.manager},
...
function renderEconomyInitiationApplicationList(){
  ...
  return months.map(month=>`<section class="economy-init-month"><div class="economy-init-month-title"><strong>${month}</strong><span>⌄</span></div>${economyInitiationApplications.filter(x=>x.month===month).map(app=>`<button class="economy-init-application-card ${app.id===economyInitiationState.selectedId?'active':''}" onclick="selectEconomyInitiationApplication('${app.id}')"><div><strong>${app.company}开项申请</strong>${economyInitiationStatusTag(app.status)}</div><p><span>提交组织</span><b>${app.company}</b></p><p><span>提交时间</span><b>${app.submitTime}</b></p><p><span>当前节点</span><b>${app.currentNode}</b></p></button>`).join("")}</section>`).join("");
}
...
function openEconomyInitiationHistoryProjects(){
  ...
  openModal("选择历史项目",`<div class="economy-init-history-tip">仅展示上月之前尚未完成开项的历史项目，支持多选加入本月清单。</div><div class="table-wrap"><table style="min-width:900px"><thead><tr><th style="width:60px">选择</th><th>生产项目编号</th><th>生产项目名称</th><th>分公司</th><th>项目经理</th><th>立项完成日期</th></tr></thead><tbody>${available.length?available.map(x=>`<tr><td style="text-align:center"><input type="checkbox" onchange="toggleEconomyInitiationHistorySelection('${x.id}',this.checked)"/></td><td>${x.projectCode}</td><td>${x.projectName}</td><td>${x.branch}</td><td>${x.manager}</td><td>${x.establishDate}</td></tr>`).join(""):'<tr><td colspan="6" class="empty-state">暂无可选择的历史项目</td></tr>'}</tbody></table></div>`,`<button class="btn" onclick="closeModal()">取消</button><button class="btn primary" onclick="confirmEconomyInitiationHistorySelection()">确定</button>`,"large");
}
...
function renderEconomyProjectInitiationPage(){
  ...
  listPage.innerHTML=`<div class="economy-init-page"><aside class="card economy-init-sidebar"><div class="card-hd"><div class="card-title">子公司开项申请</div></div><div class="economy-init-auto-tip">每月自动生成，自动纳入上月新立项项目</div><div class="economy-init-application-list">${renderEconomyInitiationApplicationList()}</div></aside><main class="economy-init-main"><section class="card economy-init-header-card"><div><h2>${app.month} ${app.company}开项申请项目清单</h2><p>提交组织：${app.applicant}　提交时间：${app.submitTime}　当前节点：${app.currentNode}</p></div><div class="actions">${actions}</div></section>${renderUnifiedQueryCard(queryFields,{gridClass:"search-grid economy-init-search-grid",queryFn:"syncEconomyInitiationFilters()",resetFn:"resetEconomyInitiationFilters()"})}${renderUnifiedTableCard({title:"开项申请项目清单",tableKey:"economyInitiationProjects",tableId:"economyInitiationProjectTable",theadId:"economyInitiationProjectThead",tbodyId:"economyInitiationProjectTbody",totalId:"economyInitiationProjectTotal",total:list.length,renderFnName:"renderEconomyProjectInitiationPage",beforeActions:app.status==="待子公司提交"?`<button class="btn" onclick="openEconomyInitiationHistoryProjects()">选择历史项目</button>`:"",refreshAction:"renderEconomyProjectInitiationPage();showToast('已刷新开项申请清单')",exportAction:"showToast('导出成功：开项申请项目清单.xlsx')"})}${renderEconomyInitiationProgress(app)}</main></div>`;
