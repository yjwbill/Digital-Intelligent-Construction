...
function renderSafetyEvaluationManagePage(name){
  ...
  if(name==="评价任务管理"){
    safetyEvalTaskCurrentMode=false;
    return renderSafetyEvaluationTaskPage();
  }
  if(name==="安全评价任务"){
    safetyEvalTaskCurrentMode=true;
    return renderSafetyEvaluationTaskPage();
  }
...
function getSafetyEvalModelDefaultWeights(){
  return safetyEvalModelDimensionTemplate.map(group=>group.children.map(item=>{
    const groupScore=Number(group.score)||getSafetyEvalModelGroupScore(group)||1;
    return Number(((Number(item.score)||0)/groupScore*100).toFixed(1));
  }));
}

function normalizeSafetyEvalModelWeights(weights){
  return weights.map(group=>group.map(value=>Math.min(100,Math.max(0,Number(value)||0))));
}

function getSafetyEvalCurrentModelWeights(modelId){
  const key=String(modelId);
  if(!safetyEvalCurrentModelWeights[key])safetyEvalCurrentModelWeights[key]=getSafetyEvalModelDefaultWeights();
  return safetyEvalCurrentModelWeights[key];
}

function cloneSafetyEvalModelWeights(weights){
  return weights.map(group=>group.slice());
}

function getSafetyEvalModelDisplayWeights(){
  if(safetyEvalModelDetailContext.editing&&safetyEvalModelEditDraft)return safetyEvalModelEditDraft;
  return getSafetyEvalCurrentModelWeights(safetyEvalModelDetailContext.modelId);
}

function getSafetyEvalModelIndicatorWeight(groupIndex,itemIndex){
  return Number(getSafetyEvalModelDisplayWeights()?.[groupIndex]?.[itemIndex])||0;
}
...
function getSafetyEvalModelGroupWeightTotal(groupIndex){
  return (getSafetyEvalModelDisplayWeights()?.[groupIndex]||[]).reduce((sum,value)=>sum+(Number(value)||0),0);
}

function updateSafetyEvalModelWeight(groupIndex,itemIndex,value){
  if(!safetyEvalModelDetailContext.editing||!safetyEvalModelEditDraft)return;
  const normalized=Math.min(100,Math.max(0,Number(value)||0));
  safetyEvalModelEditDraft[groupIndex][itemIndex]=normalized;
  const score=document.querySelector(`[data-model-indicator-score="${groupIndex}-${itemIndex}"]`);
  if(score)score.textContent=formatSafetyEvalModelScore(getSafetyEvalModelIndicatorScore(groupIndex,itemIndex));
  const treeScore=document.querySelector(`[data-model-tree-score="${groupIndex}-${itemIndex}"]`);
  if(treeScore)treeScore.textContent=`${formatSafetyEvalModelScore(getSafetyEvalModelIndicatorScore(groupIndex,itemIndex))}分`;
  const total=document.querySelector(`[data-model-group-weight-total="${groupIndex}"]`);
  const totalValue=getSafetyEvalModelGroupWeightTotal(groupIndex);
  document.querySelectorAll(`[data-model-weight-group="${groupIndex}"]`).forEach(field=>field.classList.toggle("over-limit",totalValue>100.01));
  if(total){
...
function renderSafetyEvalModelIndicatorPanel(activeIndex=0){
  ...
  return `
    ...
    <table class="safety-eval-detail-table model-detail-indicator-table ${safetyEvalModelCurrentMode?"current-safety-model-indicator-table":""}">
      <thead>
        <tr>${currentModelHeaders}<th>指标编码</th><th>指标名称</th><th>指标类型</th><th>数据来源</th><th>计算方式</th>${currentModelWeightHeader}<th>分值</th>${legacyDeductHeader}</tr>
      </thead>
      <tbody>
        ${group.children.map((item,index)=>`
          <tr>
            ${safetyEvalModelCurrentMode?`<td>${index+1}</td>`:""}
            <td>${item.code}</td>
            <td>${item.name}</td>
            <td>${tag(item.type,item.type==="评价因子"?"blue":"purple")}</td>
            <td>${item.source}</td>
            <td>${tag(item.method,item.method==="自动计算"?"green":item.method==="规则计算"?"orange":"gray")}</td>
            ${safetyEvalModelCurrentMode?`<td>${safetyEvalModelDetailContext.editing?`<div class="model-weight-input ${getSafetyEvalModelGroupWeightTotal(activeIndex)>100.01?"over-limit":""}" data-model-weight-group="${activeIndex}"><input type="number" min="0" max="100" step="0.1" value="${formatSafetyEvalModelWeight(getSafetyEvalModelIndicatorWeight(activeIndex,index))}" oninput="this.value=Math.min(100,Math.max(0,Number(this.value)||0));updateSafetyEvalModelWeight(${activeIndex},${index},this.value)"/><span>%</span></div>`:`${formatSafetyEvalModelWeight(getSafetyEvalModelIndicatorWeight(activeIndex,index))}%`}</td>`:""}
            <td ${safetyEvalModelCurrentMode?`data-model-indicator-score="${activeIndex}-${index}"`:""}>${formatSafetyEvalModelScore(safetyEvalModelCurrentMode?getSafetyEvalModelIndicatorScore(activeIndex,index):item.score)}</td>
            ${safetyEvalModelCurrentMode?"":`<td>${item.deduct}</td>`}
          </tr>
        `).join("")}
      </tbody>
    </table>
...
function openSafetyEvalModelDetail(id,editing=false){
  ...
  openModal(canEdit?"编辑模型":"模型详情",html,canEdit
    ?`<button class="btn" onclick="closeModal()">取消</button><button class="btn primary" onclick="saveSafetyEvalModelWeights()">保存</button>`
    :`<button class="btn" onclick="closeModal()">关闭</button>`,"large");
}

function openSafetyEvalModelEdit(id){
  openSafetyEvalModelDetail(id,true);
}
...
const safetyEvalResultOptions={
  ...
  models:["三层安全评价模型组合","集团安全评价模型","施工项目周评价模型","供应链安全履约评价模型","关键岗位履职评价模型","专项风险评价模型"],
...
const safetyEvalResultRows=[
  ...
  ["RES-202607-006","2026-07","上海XXX劳务有限公司","供应链","上海隧道","轨交分公司","供应链安全履约评价模型","v1.6",67,"风险较高",13.5,12.0,14.0,13.0,14.5,-1.5,7,"待确认","2026-07-01 10:30"],
  ["RES-202607-007","2026-07","项目经理岗位","岗位","市政集团","上海分公司","关键岗位履职评价模型","v1.2",91,"风险可控",18.0,18.5,19.0,17.5,18.0,4.1,2,"待确认","2026-07-01 10:36"],
  ...
  ["RES-202607-010","2026-07","专职安全员岗位","岗位","上海隧道","轨交分公司","关键岗位履职评价模型","v1.2",88,"风险可控",17.5,17.0,18.5,17.0,18.0,3.6,2,"待确认","2026-07-01 10:55"],
...
function confirmSafetyEvalResult(id){
  const row=safetyEvalResultRows.find(item=>item.id===Number(id));
  if(!row||row.resultStatus!=="待确认")return;
  const replacedRows=safetyEvalResultRows.filter(item=>item.id!==row.id&&item.evalMonth===row.evalMonth&&item.resultStatus==="已确认");
  replacedRows.forEach(item=>{item.resultStatus="待确认";item.confirmTime="-";});
  row.resultStatus="已确认";
  row.confirmTime="2026-07-23 18:35";
  showToast(replacedRows.length
    ?`已确认 ${row.resultCode}；同一期结果 ${replacedRows.map(item=>item.resultCode).join("、")} 已取消确认`
    :`已确认结果：${row.resultCode}`);
  renderSafetyEvaluationResultPage();
}
...
const safetyEvalCurrentTaskRows=[
  [201,"TASK-202607-001","安全评价月度任务","2026-07","自动","未执行",0,9,9,6,"-","-","王安全","2026-07-01 08:00"],
  [202,"TASK-202606-001","安全评价月度任务","2026-06","自动","执行中",45,10,10,6,"2026-07-23 08:30","-","李明","2026-06-01 08:00"],
  [203,"TASK-202605-001","安全评价月度任务","2026-05","手动","已执行",100,8,8,6,"2026-05-31 09:00","2026-05-31 09:08","张强","2026-05-01 08:00"],
  [204,"TASK-202604-001","安全评价月度任务","2026-04","自动","已执行",100,7,7,5,"2026-04-30 08:30","2026-04-30 08:42","王安全","2026-04-01 08:00"]
].map(row=>({id:row[0],taskCode:row[1],taskName:row[2],taskPeriod:row[3],modelName:"三层安全评价模型组合",modelId:"三层安全评价模型组合",modelNames:["项目安全评价模型","分公司安全评价模型","子公司安全评价模型"],evaluationType:"项目",cycleType:"月度",executeMode:row[4],taskStatus:row[5],progress:row[6],projectCount:row[7],branchCount:row[8],companyCount:row[9],startTime:row[10],endTime:row[11],creator:row[12],createTime:row[13],scheduledTime:row[4]==="自动"?`${row[3]}-28 08:30`:"-",taskDescription:`${row[3]}安全评价任务，统一调用项目、分公司、子公司三个层级模型完成月度安全诊断。`}));

let safetyEvalTaskCurrentMode=false;
const safetyEvalCurrentTaskTimers={};

function getActiveSafetyEvalTaskRows(){
  return safetyEvalTaskCurrentMode?safetyEvalCurrentTaskRows:safetyEvalTaskRows;
}

function getActiveSafetyEvalTaskOptions(){
  return safetyEvalTaskCurrentMode?safetyEvalCurrentTaskOptions:safetyEvalTaskOptions;
}
...
function safetyEvalCurrentTaskStatusTag(value){
  return tag(value,({未执行:"gray",执行中:"blue",已执行:"green"})[value]||"gray");
}
...
function renderSafetyEvalCurrentTaskActions(row){
  ...
  actions.push(`<a class="link" onclick="openSafetyEvalCurrentTaskEdit(${row.id})">编辑</a>`);
  ...
  if(row.taskStatus==="已执行")actions.push(`<a class="link" onclick="executeSafetyEvalCurrentTask(${row.id},true)">重新执行</a>`);
  return actions.join("\n");
}
...
function getSafetyEvalTaskCreateModels(){
  return (safetyEvalTaskCurrentMode?safetyEvalCurrentModelRows:safetyEvalModelRows).filter(row=>row.modelStatus!=="已停用");
}
...
function getSafetyEvalTaskObjectsByModel(model){
  ...
  if(safetyEvalTaskCurrentMode&&type==="子公司")return getOrganizationCompanies().map((name,index)=>({objectCode:`COMP-${String(index+1).padStart(3,"0")}`,objectName:name,orgName:name,parentOrg:"隧道股份",status:"启用"}));
  if(safetyEvalTaskCurrentMode&&type==="分公司")return getOrganizationPairs(10).map((pair,index)=>({objectCode:`BRANCH-${String(index+1).padStart(3,"0")}`,objectName:pair[1],orgName:pair[1],parentOrg:pair[0],status:"启用"}));
...
function getSafetyEvalTaskDefaultName(modelName,cycleType){
  ...
  const model=getSafetyEvalTaskCreateModels().find(row=>row.modelName===modelName) || getSafetyEvalTaskDefaultModel();
...
function renderSafetyEvalCurrentTaskSelectionHint(projectCount=0,branchCount=0,companyCount=0,dynamic=true){
  return `<div style="display:flex;align-items:center;gap:14px;margin-left:14px;font-size:13px;font-weight:400;color:var(--sub);white-space:nowrap">
    ...
    <span>已选项目 <strong ${dynamic?'id="setCurrentTaskProjectCount"':""} style="color:var(--primary)">${projectCount}</strong> 个</span>
    <span>涉及分公司 <strong ${dynamic?'id="setCurrentTaskBranchCount"':""} style="color:var(--primary)">${branchCount}</strong> 家</span>
    <span>涉及子公司 <strong ${dynamic?'id="setCurrentTaskCompanyCount"':""} style="color:var(--primary)">${companyCount}</strong> 家</span>
...
function openSafetyEvalCurrentTaskCreateModal(editId=null){
  const editingRow=editId===null?null:safetyEvalCurrentTaskRows.find(item=>item.id===Number(editId));
  const allProjects=getSafetyEvalCurrentTaskSelectableProjects();
  const editingCodes=editingRow?.selectedProjectCodes?.length
    ?editingRow.selectedProjectCodes
    :editingRow?allProjects.slice(0,Math.min(editingRow.projectCount,allProjects.length)).map(item=>item.code):[];
  safetyEvalCurrentTaskSelectedCodes=new Set(editingCodes);
  Object.keys(safetyEvalCurrentTaskProjectFilter).forEach(key=>{safetyEvalCurrentTaskProjectFilter[key]="";});
  openModal(editingRow?"编辑安全评价任务":"新增安全评价任务",`
    ...
    <div class="detail-group">
      ...
      <div class="detail-group-body">
        <div class="search-grid">
          <div class="form-item"><label>任务名称</label><input class="input" id="setCurrentTaskName" value="${escapeAttr(editingRow?.taskName||"2026年07月安全评价任务")}"/></div>
          <div class="form-item"><label>任务期数</label><input class="input" type="month" id="setCurrentTaskPeriod" value="${escapeAttr(editingRow?.taskPeriod||"2026-07")}"/></div>
          <div class="form-item"><label>执行方式</label><select class="select" id="setCurrentTaskExecuteMode" onchange="syncSafetyEvalCurrentTaskExecuteTime()"><option value="自动" ${editingRow?.executeMode!=="手动"?"selected":""}>自动</option><option value="手动" ${editingRow?.executeMode==="手动"?"selected":""}>手动</option></select></div>
          <div class="form-item" id="setCurrentTaskExecuteTimeField" style="${editingRow?.executeMode==="手动"?"display:none":""}"><label>执行时间</label><input class="input" type="datetime-local" id="setCurrentTaskExecuteTime" value="${escapeAttr(editingRow?.scheduledTime?.replace(" ","T")||"2026-07-24T08:30")}"/></div>
          <div class="form-item" style="grid-column:1/-1"><label>任务描述</label><textarea class="input" id="setCurrentTaskDesc" style="min-height:68px;resize:vertical;padding-top:8px" placeholder="请输入任务描述">${editingRow?.taskDescription||""}</textarea></div>
        </div>
      </div>
    </div>
    <div class="detail-group">
      <div class="detail-group-header" style="justify-content:flex-start"><div class="detail-group-title">选择评价项目</div>${renderSafetyEvalCurrentTaskSelectionHint()}</div>
      <div class="detail-group-body" id="setCurrentTaskProjectChooser">${renderSafetyEvalCurrentTaskProjectChooser()}</div>
    </div>
  `,`<button class="btn" onclick="closeModal()">取消</button><button class="btn primary" onclick="saveSafetyEvalCurrentTaskCreate(${editingRow?.id??"null"})">保存</button>`,"large");
  syncSafetyEvalCurrentTaskSelectionSummary();
}

function openSafetyEvalCurrentTaskEdit(id){
...
function openSafetyEvalCurrentTaskResults(id){
  const task=safetyEvalCurrentTaskRows.find(item=>item.id===Number(id));
  if(!task)return;
  const rows=safetyEvalResultRows.filter(item=>item.taskCode===task.taskCode);
  openModal(`评价结果—${task.taskPeriod}`,`
    <div class="detail-group">
      <div class="detail-group-header"><div class="detail-group-title">任务信息</div><span>共 ${rows.length} 条执行结果</span></div>
      <div class="detail-group-body"><div class="info-grid">
        <div class="info-item"><div class="info-label">任务编号</div><strong>${task.taskCode}</strong></div>
        <div class="info-item"><div class="info-label">任务名称</div><strong>${task.taskName}</strong></div>
        <div class="info-item"><div class="info-label">任务期数</div><strong>${task.taskPeriod}</strong></div>
      </div></div>
    </div>
    <div class="detail-group">
      <div class="detail-group-header"><div class="detail-group-title">评价结果</div></div>
      <div class="detail-group-body"><div class="table-wrap roster-table-wrap" style="max-height:420px;overflow:auto"><table style="min-width:1280px">
        <thead><tr><th style="width:70px;text-align:center">序号</th><th style="width:150px">结果编号</th><th style="width:210px">评价任务</th><th style="width:110px;text-align:center">评价对象数</th><th style="width:110px;text-align:center">评价指标数</th><th style="width:100px;text-align:center">综合得分</th><th style="width:110px;text-align:center">评价结果</th><th style="width:170px;text-align:center">评价时间</th><th style="width:170px;text-align:center">确认时间</th><th style="width:150px;text-align:center">操作</th></tr></thead>
        <tbody>${rows.map((row,index)=>`<tr><td style="text-align:center">${index+1}</td><td>${row.resultCode}</td><td>${row.taskName}</td><td style="text-align:center">${row.evaluatedObjectCount}</td><td style="text-align:center">${row.indicatorCount}</td><td style="text-align:center">${row.totalScore}</td><td style="text-align:center">${safetyEvalResultStatusTag(row.resultStatus)}</td><td style="text-align:center">${row.calculateTime}</td><td style="text-align:center">${row.confirmTime}</td><td style="text-align:center"><a class="link" onclick="showSafetyEvalResultDetail(${row.id})">查看详情</a><a class="link" onclick="exportSafetyEvalResult(${row.id})">导出报告</a></td></tr>`).join("")||`<tr><td colspan="10" style="text-align:center;color:var(--muted);height:80px">该任务尚未生成评价结果</td></tr>`}</tbody>
      </table></div></div>
    </div>
  `,`<button class="btn primary" onclick="closeModal()">关闭</button>`,"large");
}
function syncSafetyEvalCurrentTaskExecuteTime(){
  ...
  const field=document.getElementById("setCurrentTaskExecuteTimeField");
  if(field)field.style.display=manual?"none":"";
  if(time){if(manual)time.value="";else if(!time.value)time.value="2026-07-24T08:30";}
}

function saveSafetyEvalCurrentTaskCreate(editId=null){
  const selected=getSafetyEvalCurrentTaskSelectedProjects();
  ...
  const taskDescription=document.getElementById("setCurrentTaskDesc")?.value.trim()||"";
  ...
  const duplicate=safetyEvalCurrentTaskRows.find(item=>item.taskPeriod===taskPeriod&&item.id!==Number(editId));
  if(duplicate){showToast(`${taskPeriod} 已存在评价任务，同一期只允许创建一条任务`);return;}
  const editingRow=editId===null?null:safetyEvalCurrentTaskRows.find(item=>item.id===Number(editId));
  ...
  if(editingRow){
    Object.assign(editingRow,savedRow,{id:editingRow.id,taskCode:editingRow.taskCode,taskStatus:editingRow.taskStatus,startTime:editingRow.startTime,endTime:editingRow.endTime,createTime:editingRow.createTime,creator:editingRow.creator});
  }else{
    safetyEvalCurrentTaskRows.unshift(savedRow);
  }
  closeModal();
  safetyEvalTaskState.page=1;
  renderSafetyEvaluationTaskPage();
  showToast(`${editingRow?"编辑":"新增"}成功，已选择${selected.length}个项目并关联三个评价模型`);
}
    ...
    <div class="detail-group">
      ...
      <div class="detail-group-body">
        <div class="search-grid">
          ...
          ${safetyEvalTaskCurrentMode?`<div class="form-item"><label>任务期数</label><input class="input" type="month" id="setCreateTaskPeriod" value="2026-07"/></div>`:""}
...
function saveSafetyEvalTaskCreate(){
  ...
  const activeRows=getActiveSafetyEvalTaskRows();
  const id=Math.max(0,...activeRows.map(row=>row.id))+1;
  const code=`TASK-202607-${String(id).padStart(3,"0")}`;
  const evaluationType=getSafetyEvalTaskObjectType(model);
  const taskPeriod=document.getElementById("setCreateTaskPeriod")?.value||"2026-07";
  activeRows.unshift({
    id,
    taskCode:code,
    taskName,
    ...
    cycleType:cycle,
    executeMode,
    taskStatus:safetyEvalTaskCurrentMode?"未执行":"未开始",
    progress:0,
    objectCount:selected.length,
    finishedObjectCount:0,
    failedObjectCount:0,
    projectCount:evaluationType==="项目"?selected.length:0,
    branchCount:evaluationType==="分公司"?selected.length:0,
    companyCount:evaluationType==="子公司"?selected.length:0,
...
function getSafetyEvalTaskFilteredRows(){
  ...
  return getActiveSafetyEvalTaskRows().filter(row=>{
...
function exportSafetyEvalTask(id){
  const row=getActiveSafetyEvalTaskRows().find(item=>item.id===Number(id));
  showToast(`导出结果成功：${row?.taskName || id}`);
}

function appendSafetyEvalTaskExecutionResult(task){
  const id=Math.max(0,...safetyEvalResultRows.map(item=>Number(item.id)||0))+1;
  const sequence=safetyEvalResultRows.filter(item=>item.evalMonth===task.taskPeriod&&item.taskCode===task.taskCode).length+1;
  const score=Math.max(60,Math.min(98,82+(sequence%4)*2-task.projectCount%5));
  safetyEvalResultRows.unshift({
    id,
    resultCode:`RES-${task.taskPeriod.replace("-","")}-${String(id).padStart(3,"0")}`,
    evalMonth:task.taskPeriod,
    objectName:`${task.projectCount}个项目评价范围`,
    objectType:"项目",
    companyName:task.companyCount===1?"单一子公司":"多家子公司",
    branchName:task.branchCount===1?"单一分公司":"多家分公司",
    modelName:"三层安全评价模型组合",
    modelId:"三层安全评价模型组合",
    modelVersion:"当前版本",
    ...
    siteControlScore:Number((score*.2).toFixed(1)),
    orgBehaviorScore:Number((score*.2).toFixed(1)),
    techEconomicScore:Number((score*.2).toFixed(1)),
    traceabilityScore:Number((score*.2).toFixed(1)),
    monthOnMonthChange:0,
    deductionFactorCount:Math.max(0,100-score),
    resultStatus:"待确认",
    calculateTime:"2026-07-23 18:01",
    confirmTime:"-"
  });
}

function executeSafetyEvalCurrentTask(id,isRetry=false){
  const row=safetyEvalCurrentTaskRows.find(item=>item.id===Number(id));
  if(!row)return;
  clearTimeout(safetyEvalCurrentTaskTimers[row.id]);
  row.taskStatus="执行中";
  row.progress=20;
  row.startTime="2026-07-23 18:00";
  row.endTime="-";
  renderSafetyEvaluationTaskPage();
  showToast(`${isRetry?"已重新执行":"已开始执行"}：${row.taskName}`);
  safetyEvalCurrentTaskTimers[row.id]=setTimeout(()=>{
    row.taskStatus="已执行";
    row.progress=100;
    row.endTime="2026-07-23 18:01";
    appendSafetyEvalTaskExecutionResult(row);
    const onCurrentTaskPage=safetyEvalTaskCurrentMode&&document.querySelector(".module-title")?.textContent?.includes("安全评价任务");
    if(onCurrentTaskPage){renderSafetyEvaluationTaskPage();showToast(`诊断执行完成，已生成第 ${safetyEvalResultRows.filter(item=>item.taskCode===row.taskCode).length} 条执行结果`);}
  },1200);
}

function renderSafetyEvalTaskTable(rows){
  const tableKey=safetyEvalTaskCurrentMode?"safetyEvalCurrentTask":"safetyEvalTask";
  const columns=getVisibleColumns(tableKey);
  return `
    <table class="safety-eval-task-table" style="min-width:${getTableMinWidth(tableKey)}px">
      <thead>
        <tr>
          ${renderTableHeaderByColumns(tableKey)}
        </tr>
      </thead>
      <tbody>
        ${rows.map((row,index)=>`
          <tr>
            ${columns.map(col=>`
              <td class="${getTableColumnClass(tableKey,col,columns)}" data-column-key="${escapeAttr(col.key)}" style="${getTableColumnStickyStyle(tableKey,col,columns)}width:${col.width}px;min-width:${col.width}px;max-width:${col.width}px;text-align:${col.align||"left"}">${col.render(row,index)}</td>
            `).join("")}
          </tr>
        `).join("") || `<tr><td colspan="${columns.length}" style="text-align:center;color:var(--muted);height:80px">暂无数据</td></tr>`}
      </tbody>
...
function renderSafetyEvaluationTaskPage(){
  ...
  const options=getActiveSafetyEvalTaskOptions();
  const tableKey=safetyEvalTaskCurrentMode?"safetyEvalCurrentTask":"safetyEvalTask";
  listPage.innerHTML=`
    <div class="compact-title-row">
      <div class="module-title">安全评价 / ${safetyEvalTaskCurrentMode?"安全评价任务":"评价任务管理"}</div>
    </div>
    ${renderUnifiedQueryCard(`
      ...
      <div class="form-item"><label>评价模型</label><select class="select" id="setModel">${renderSafetyEvalTaskOptions(options.models,safetyEvalTaskState.modelId,"全部")}</select></div>
      <div class="form-item"><label>评价对象</label><select class="select" id="setType">${renderSafetyEvalTaskOptions(options.types,safetyEvalTaskState.evaluationType,"全部")}</select></div>
      <div class="form-item"><label>任务状态</label><select class="select" id="setStatus">${renderSafetyEvalTaskOptions(options.status,safetyEvalTaskState.taskStatus,"全部")}</select></div>
    ...
    <section class="card table-card safety-eval-task-card">
      <div class="card-hd">
        ...
        <div class="actions">
          ...
          ${safetyEvalTaskCurrentMode?"":`<button class="btn" onclick="showToast('批量执行功能演示')">批量执行</button>`}
          <button class="btn" onclick="showToast('导出成功')">导出</button>
          <button class="column-setting-icon-btn" title="列配置" onclick="openColumnSetting('${tableKey}','renderSafetyEvaluationTaskPage')">⚙</button>
        </div>
      </div>
      <div class="table-wrap roster-table-wrap">
