...
const economyInitiationState={selectedId:"init-202607-st",projectName:"",projectCode:"",orderCode:"",branch:"",manager:"",historySelected:[],expandedMonths:new Set([economyInitiationLatestMonth])};
...
function economyInitiationStatusTag(status){
  ...
  const label={"待子公司提交":"待提交","产运部审批":"待审批"}[status]||status;
...
function renderEconomyInitiationProgress(app){
  ...
  const steps=[
    {name:"流程创建",actor:"系统创建",role:"系统",time:creationTime,reached:true},
    {name:"子公司申请",actor:"提交人",role:"子公司经济审核岗",time:app.submitTime,reached:true},
    {name:"产运部审批",actor:"审批人",role:"产运部审批岗",time:app.status==="已完成"?(app.approveTime||"--"):"--",reached:app.status!=="待子公司提交"},
    {name:"结束",actor:"流程结束",role:"系统",time:app.status==="已完成"?(app.approveTime||"--"):"--",reached:app.status==="已完成"}
  ];
  return `<div class="economy-init-flow" aria-label="审批流程"><div class="economy-init-flow-track">${steps.map((step,index)=>`${index?'<i class="economy-init-flow-arrow" aria-hidden="true"></i>':''}<div class="economy-init-flow-node ${step.reached?'reached':'pending'}"><div class="economy-init-flow-label"><b>${index+1}</b><strong>${step.name}</strong></div><div class="economy-init-flow-detail"><span>${step.actor}<em>${step.role}</em></span><time>${step.time||"--"}</time></div></div>`).join("")}</div></div>`;
}
