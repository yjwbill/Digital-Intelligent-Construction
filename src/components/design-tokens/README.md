...
- 平台主色：`#165DFF`，统一引用 `--dsc-color-primary`。
- 页面、容器、文字、边框及成功/警告/危险色详见 Semantic / Color。

## 经济预警

- 红色 `#FF0013`、橙色 `#FF9933`、黄色 `#FFF44F`、蓝色 `#4596E2`。
- 通用反馈色不得替代经济预警业务色。

## 表格 Table

- 单行表头 `44px`；双行或多级表头每层 `40px`。
- 数据行统一 `40px`，不保留 `48px/52px`。

## 圆角与容器

- 普通 Card 和常规容器 `8px`；强调容器和 Modal `12px`。
- 历史 `16px` 及以上归并为 `12px`；`10px/6px` 归并为 `8px`。

## 图表 Chart

- 数据折线、辅助线、平均线统一 `1px`。
- 坐标轴 `1px`，网格线 `0.5px`。
- 紧凑型柱宽 `18px`，标准型柱宽 `28px`。
